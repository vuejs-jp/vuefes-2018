<template>
  <form class="email-registration-form" @submit.prevent="registerEmail" ref="form" v-if="!isRegistered">
    <div class="form-content">
      ...
      <button class="base-button" type="submit" :disabled="!isValid || status.isLoading">
...
input {
  margin-bottom: 10px;
  width: calc(100% - 40px);
  max-width: calc(#{$base-button__max-width} - 40px);
  height: 60px;
  padding-left: 18px;
  padding-right: 18px;
  font-size: 18px;
...
.base-button {
  ...
  width: 100%;
  max-width: $base-button__max-width;
...
@media screen and (min-width: $layout-breakpoint--is-small-up) {
  ...
  .form-content {
    width: $base-button__max-width;
  }

  input {
    display: block;
    width: calc(#{$base-button__max-width} - 40px);
