export const state = () => ({
  ...
  silverSponsors: [
    ...
    },
    {
      ...
      banner: 'pepabo.png',
      description: [
        'GMOペパボ株式会社は「もっとおもしろくできる」を企業理念に、レンタルサーバー「ロリポップ！」や国内最大のハンドメイドマーケット「minne」など様々なサービスを提供しています。新規開発から既存のサービスまで複数のサービスに渡って Vue.js の導入や刷新を進めており、「いるだけで成長できる環境」をコンセプトにエンジニアの人材育成やコミュニティへの支援を積極的に行っています。',
      ],
    },
    {
      name: 'エムスリー株式会社',
...
export const getters = {
  ...
  platinumSponsors: (state) => state.platinumSponsors,
  goldSponsors: (state) => state.goldSponsors,
  specialSponsors: (state) => state.specialSponsors,
  silverSponsors: (state) => state.silverSponsors,
  bronzeSponsors: (state) => state.bronzeSponsors,
  networkSponsors: (state) => state.networkSponsors,
  lunchSponsors: (state) => state.lunchSponsors,
  beerSponsors: (state) => state.beerSponsors,
  drinkSponsors: (state) => state.drinkSponsors,
  toolSponsors: (state) => state.toolSponsors,
  videoSponsors: (state) => state.videoSponsors,
  mediaSponsors: (state) => state.mediaSponsors,
  stickerSponsors: (state) => state.stickerSponsors,
}
